...
- **#56** — make sure that the readme is up to date — delivered via PR #57 (1d2b7cb3); 3 files changed.
- **58** — Investigated feature request #58 (add truncateWords to text-utilities). Found that the function is already fully implemented — `truncateWords` exists in the text-utilities module with the exact signature, behavior, and tests the issue requests. All acceptance criteria are covered: zero/negative maxWords, empty/whitespace input, word-boundary truncation with ellipsis, and reuse of existing tokenization. No code changes needed — the work is already complete.
